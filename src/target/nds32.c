...
/* get register value internally */
int nds32_get_mapped_reg64(struct nds32 *nds32, unsigned regnum, uint64_t *value)
{
	struct reg_cache *reg_cache = nds32->core_cache;
	struct reg *r;
...
/** set register internally */
int nds32_set_mapped_reg64(struct nds32 *nds32, unsigned regnum, uint64_t value)
{
	struct reg_cache *reg_cache = nds32->core_cache;
	struct reg *r;
	uint8_t set_value[4];
...
int nds32_read_buffer(struct target *target, long unsigned int address,
		uint32_t size, uint8_t *buffer)
{
	...
	LOG_DEBUG("READ BUFFER: ADDR %ld" PRIx32 "  SIZE %08" PRIx32,
			address,
			size);
...
int nds32_read_memory(struct target *target, long unsigned int address,
...
int nds32_read_phys_memory(struct target *target, uint64_t address,
		uint32_t size, uint32_t count, uint8_t *buffer)
{
	struct aice_port_s *aice = target_to_aice(target);
	struct nds32 *nds32 = target_to_nds32(target);
...
int nds32_write_buffer(struct target *target, long unsigned int address,
		uint32_t size, const uint8_t *buffer)
{
	...
	LOG_DEBUG("WRITE BUFFER: ADDR %ld" PRIx32 "  SIZE %08" PRIx32,
			address,
			size);
...
int nds32_write_memory(struct target *target, long unsigned int address,
...
int nds32_write_phys_memory(struct target *target, uint64_t address,
		uint32_t size, uint32_t count, const uint8_t *buffer)
{
	struct aice_port_s *aice = target_to_aice(target);
	struct nds32 *nds32 = target_to_nds32(target);
...
int nds32_virtual_to_physical(struct target *target, uint64_t address, uint64_t *physical)
{
	struct nds32 *nds32 = target_to_nds32(target);
	...
	if ((icache->line_size != 0) && (icache->enable == true)) {
		...
		for (cur_address = address, cur_line = start_line ;
			...
			if (ERROR_FAIL == target->type->virt2phys(target, cur_address,
						(long unsigned int *)&physical_addr))
				return ERROR_FAIL;
...
int nds32_step(struct target *target, int current,
		long unsigned int address, int handle_breakpoints)
{
	...
	LOG_DEBUG("STEP PC %ld" PRIx32 "%s", address, !current ? "!" : "");
...
int nds32_resume(struct target *target, int current,
		long unsigned int address, int handle_breakpoints, int debug_execution)
{
	LOG_DEBUG("current %d address %ld" PRIx32
	...
	LOG_DEBUG("RESUME PC %ld" PRIx32 "%s", address, !current ? "!" : "");
...
int nds32_get_gdb_fileio_info(struct target *target, struct gdb_fileio_info *fileio_info)
{
	...
	switch (syscall_id) {
		case NDS32_SYSCALL_EXIT:
			...
			nds32_get_mapped_reg64(nds32, R0, &(fileio_info->param_1));
			break;
		case NDS32_SYSCALL_OPEN:
			{
				...
				nds32_get_mapped_reg64(nds32, R1, &(fileio_info->param_3));
				nds32_get_mapped_reg64(nds32, R2, &(fileio_info->param_4));
		...
		case NDS32_SYSCALL_READ:
			...
			nds32_get_mapped_reg64(nds32, R0, &(fileio_info->param_1));
			nds32_get_mapped_reg64(nds32, R1, &(fileio_info->param_2));
			nds32_get_mapped_reg64(nds32, R2, &(fileio_info->param_3));
			break;
