...
static int nds32_v2_activate_hardware_breakpoint(struct target *target)
{
	...
	for (bp = target->breakpoints; bp; bp = bp->next) {
		if (bp->type == BKPT_SOFT) {
			...
			LOG_DEBUG("Add hardware BP %" PRId32 " at %ld" PRIx32, hbr_index,
					bp->address);
...
static int nds32_v2_deactivate_hardware_breakpoint(struct target *target)
{
	...
	for (bp = target->breakpoints; bp; bp = bp->next) {
		...
		LOG_DEBUG("Remove hardware BP %" PRId32 " at %ld" PRIx32, hbr_index,
				bp->address);
...
static int nds32_v2_activate_hardware_watchpoint(struct target *target)
{
	...
	for (wp = target->watchpoints; wp; wp = wp->next) {
		...
		LOG_DEBUG("Add hardware watchpoint %" PRId32 " at %ld" PRIx32 " mask %08" PRIx32, wp_num,
				wp->address, wp->mask);
...
static int nds32_v2_deactivate_hardware_watchpoint(struct target *target)
{
	...
	for (wp = target->watchpoints; wp; wp = wp->next) {
		...
		LOG_DEBUG("Remove hardware watchpoint %" PRId32 " at %ld" PRIx32 " mask %08" PRIx32,
...
static int nds32_v2_read_buffer(struct target *target, long unsigned int address,
		uint32_t size, uint8_t *buffer)
{
	struct nds32 *nds32 = target_to_nds32(target);
	struct nds32_memory *memory = &(nds32->memory);
	...
	nds32_v2_translate_address(target, (unsigned int *)&address);

	return nds32_read_buffer(target, address, size, buffer);
}

static int nds32_v2_write_buffer(struct target *target, long unsigned int address,
...
static int nds32_v2_read_memory(struct target *target, long unsigned int address,
...
static int nds32_v2_write_memory(struct target *target, long unsigned int address,
